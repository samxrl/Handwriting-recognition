...
class gcForest(object):
    ...
    def window_slicing_pred_prob(self, X, window, shape_1X, y=None):
        ...
        if hasattr(self, '_mgsprf_{}'.format(window)) and y is None:
            ...
            # print('Coverage of window {}'.format(window))
            # prf.get_MSG_Coverage(sliced_X)
            # crf.get_MSG_Coverage(sliced_X)
    ...
    def _window_slicing_img(self, X, window, shape_1X, y=None, stride=1):
        ...
        # print([shape_1X[1] * i for i in range(window)])
        # print([ref_row + shape_1X[1] * i for i in range(window)])
        # print([i for i in range(window)])
        # print([ref_ind + ix + shape_1X[1] * iy
    ...
    def cascade_forest(self, X, y=None):
        ...
        if y is not None:
            ...
            self.n_layer += 1
            prf_crf_pred_ref,a,b = self._cascade_layer(X_train, y_train)
            accuracy_ref = self._cascade_evaluation(X_test, y_test)
            feat_arr = self._create_feat_arr(X_train, prf_crf_pred_ref)

            self.n_layer += 1
            prf_crf_pred_layer,a,b = self._cascade_layer(feat_arr, y_train)
            ...
            at_layer = 1
            prf_crf_pred_ref,a,b= self._cascade_layer(X, layer=at_layer)
            while at_layer < getattr(self, 'n_layer'):
                at_layer += 1
                feat_arr = self._create_feat_arr(X, prf_crf_pred_ref)
                prf_crf_pred_ref,a1,b1= self._cascade_layer(feat_arr, layer=at_layer)
            ...
            # Coverage = (a+b)
            # Coverage = Coverage/2
            # Coverage = Coverage/getattr(self, 'n_layer')
            # Coverage = Coverage/getattr(self, 'n_cascadeRF')
            #
            # print('Coverage of Cascade_forest:')
            # print(Coverage)
    ...
    def _cascade_layer(self, X, y=None, layer=0):
        ...
        prf_crf_pred = []
        prf_all_Coverage=np.zeros(10)
        crf_all_Coverage=np.zeros(10)
        ...
        if y is not None:
            ...
            for irf in range(n_cascadeRF):
                ...
                # K = 100
                #
                # prf_Coverage = np.zeros((10, K))
                # crf_Coverage = np.zeros((10, K))

                prf_Results = prf.predict_proba(X)
                crf_Results = crf.predict_proba(X)

                # for a, b in zip(prf_Results, crf_Results):
                #     for i in range(10):
                #         if a[i] != 0 and a[i] != 0.0:
                #             an = a[i]/(1.0 / K)
                #             if int(an) == K:
                #                 prf_Coverage[i][K-1] = 1
                #             else:
                #                 prf_Coverage[i][int(an)] = 1
                #         if b[i] != 0 and b[i] != 0.0:
                #             bn = b[i] / (1.0 / K)
                #             if int(bn) == K:
                #                 crf_Coverage[i][K-1] = 1
                #             else:
                #                 crf_Coverage[i][int(bn)] = 1

                # prfCoverage = (prf_Coverage != 0).sum(1)/K
                # crfCoverage = (crf_Coverage != 0).sum(1)/K
                #
                # print('Coverage with K={} at layer={}'.format(K,layer))
                # print(prfCoverage)
                # print(crfCoverage)

                # prf_all_Coverage = prf_all_Coverage+prfCoverage
                # crf_all_Coverage = crf_all_Coverage+crfCoverage

                prf_crf_pred.append(prf_Results)
                prf_crf_pred.append(crf_Results)

        return prf_crf_pred, prf_all_Coverage, crf_all_Coverage
